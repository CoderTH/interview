...
![](https://gitee.com/coderth/blogimage/raw/master/img/20210428101016.png)

首先如果c向s发送一个syn连接请求（seq：90）这个请求因为某些原因在网络中阻塞，超过一定时间，c会重新向s发送一条新的syn（seq：100），同时，旧的syn先到的话，s会相应给c一个syn+ACK，但是c通过上下问判断自己应该收到的Ack Num应该是100+1而不是90+1，所以会给s发送一个RST信号来废弃这条旧的连接，当新的syn（seq：100）到达s，这时正常建立连接